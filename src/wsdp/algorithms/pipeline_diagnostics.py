"""Offline diagnostics for comparing raw and processed CSI pipelines."""
import csv
import json
...
from .features import doppler_spectrum
...
def compute_pipeline_metrics(
    ...
    """Compute simple CPU-only proxy metrics for raw/processed CSI comparison."""
...
def _doppler_summary(csi: np.ndarray, n_fft: int, hop_length: int) -> np.ndarray:
    spectrum = doppler_spectrum(csi, n_fft=n_fft, hop_length=hop_length)
    if spectrum.size == 0:
        return np.zeros((1, 1))
    if spectrum.ndim <= 2:
        return spectrum
    return np.mean(spectrum, axis=tuple(range(2, spectrum.ndim)))
