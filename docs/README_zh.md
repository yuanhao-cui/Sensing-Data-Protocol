...
![最优模型](../img/best_models_accuracy.png)
*图 4：流水线调参与模型选择的对比*

</div>

<p align="justify"><em>橙色柱是每个数据集上通过网格搜索五个预处理步骤得到的最优流水线，四个数据集的最优组合都使用 Savitzky-Golay 去噪，模型分别为 MLP（Widar3.0 和 Gait）、ResNet1D（XRF55）和 CSITime（ElderAL-CSI）。蓝色和青色柱分别是排名第一的模型在其最优预设上的准确率和在六个预设上的平均准确率，青色柱带标准差误差棒，灰色柱为默认流水线。在四个数据集上，调节预处理流水线都比更换模型带来更大的提升。</em></p>
